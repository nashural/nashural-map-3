...
LocationTool.prototype = {
  ...
  _setupMonitor: function () {
    this._monitor.add(
      ['mapCenter', 'mapZoom', 'markerPosition'],
      this._onMapViewStateChange,
      this
    )
  },
  /**
...
LocationTool.MapView.prototype = {
  ...
  _attachHandlers: function () {
    ...
    this._marker.events.add('drag', this._onMarkerDrag, this)
  },
  /**
   * Снимаем обработчики.
   * @function
  ...
  _detachHandlers: function () {
    this._marker.events.remove('drag', this._onMarkerDrag, this)
  ...
  _onMapAction: function (e) {
    ...
    var state = this._map.action.getCurrentState(),
       ...
       */
      center = this._map.options
        .get('projection')
        .fromGlobalPixels(state.globalPixelCenter, zoom)
...
LocationTool.DOMView.prototype = {
  ...
  _setData: function (id, value) {
    this._element
      .find('#' + id)
      .val(
        $.isArray(value) ? $.map(value, this._toFixedNumber).join(', ') : value
      )
  },
