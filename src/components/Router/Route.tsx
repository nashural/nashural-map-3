...
const parsePos = (pos: string): GeoJSONCoordinates =>
  pos.split(" ").map(Number) as GeoJSONCoordinates;
...
const featuresToOptionsGroups = (
  features: GeoJSONFeature[],
  activeGroups: string[],
  groupNames: GroupNamesIndex
): object[] => {
  const featuresByGroups = groupBy(
    features,
    (feature: GeoJSONFeature): string =>
      (feature.properties as CustomProperties).group
  );
  return activeGroups
...
export const Route: FC<RouteProps> = ({ id, name, immutable, index }) => {
  ...
  const searchResultToOptionGroups = useMemo(
    () => ({
      groupId,
      features,
    }: {
  ...
  const geocoderResultToOptionGroups = useMemo(
    () => ({ GeoObjectCollection }: any) => {
      return {
        label: "Населенные пункты",
        options: GeoObjectCollection.featureMember.map(
          ({ GeoObject }: any, idx: number) => ({
        ...
        return (
          ...
          >
            <i
              ...
              className="fa fa-bars Route-draghandle"
            ></i>
            <AsyncSelect
              key={`${id}-${name}`}
